// contracts/fusion-resolver/src/core/OrderBook.ts
import {
  FusionOrder,
  OrderStatus,
  OrderFilters,
...
export class OrderBook {
  private orders = new Map<string, FusionOrder>();
  ...
  // Create a new order
  async createOrder(params: CreateOrderParams): Promise<FusionOrder> {
    ...
    try {
      ...
      const auctionDuration = config.auction.defaultDuration;
  ...
  private startAuction(orderHash: string): void {
    ...
    this.logger.info("Auction started", {
      orderHash,
      duration: config.auction.defaultDuration,
    });
  }
    ...
    if (filters) {
      if (filters.maker) {
        orders = orders.filter(
          (order) => order.maker.toLowerCase() === filters.maker!.toLowerCase()
        );
      }
      ...
      if (filters.srcChain) {
        orders = orders.filter(
          (order) =>
            order.srcChainId.toString() === filters.srcChain!.toString()
        );
      }
      ...
      if (filters.dstChain) {
        orders = orders.filter(
          (order) =>
            order.dstChainId.toString() === filters.dstChain!.toString()
        );
      }
  ...
  // Generate order hash
  private generateOrderHash(params: CreateOrderParams): string {
    const data = [
      params.maker,
      params.receiver,
      params.makerAsset,
  ...
  getStats(): OrderStats {
    ...
    const stats: OrderStats = {
      ...
      active: orders.filter((o) =>
        ["pending", "auction_active", "htlc_created"].includes(o.status)
      ).length,
  ...
  private calculateTotalVolume(orders: FusionOrder[]): string {
