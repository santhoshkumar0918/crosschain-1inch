...
export class QuoteCalculator {
  ...
  async calculateQuote(request: QuoteRequest): Promise<QuoteResponse> {
    ...
    try {
      ...
      const dstAmountBeforeSlippage = this.calculateOutputAmount(
        srcAmount,
        exchangeRate
      );
      ...
      const quote: QuoteResponse = {
        ...
        estimatedTime: this.estimateSwapTime(
          request.srcChain,
          request.dstChain
        ),
        route,
  ...
  private async estimateGasCosts(request: QuoteRequest): Promise<number> {
    ...
    // Ethereum gas costs
    if (
      request.srcChain === config.ethereum.chainId ||
      request.dstChain === config.ethereum.chainId
    ) {
  ...
  private estimateSwapTime(
    ...
    return (
      auctionTime + blockConfirmationTime + htlcCreationTime + secretRevealTime
    );
  }
