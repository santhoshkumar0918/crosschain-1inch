...
class AssetManager {
    logger = new logger_1.Logger("AssetManager");
    assets = new Map();
    constructor() {
    ...
    initializeDefaultAssets() {
        ...
        this.registerAsset({
            address: config_1.config.ethereum.htlcAddress,
            symbol: "ETH",
            decimals: liquidity_1.ASSET_DECIMALS.ETH,
            network: "ethereum",
        ...
        this.registerAsset({
            address: config_1.config.stellar.htlcContractId,
            symbol: "XLM",
            decimals: liquidity_1.ASSET_DECIMALS.XLM,
            network: "stellar",
    ...
    registerAsset(assetConfig) {
        try {
            ...
            throw new liquidity_1.LiquidityException(liquidity_1.LiquidityError.CONFIGURATION_ERROR, `Failed to register asset: ${error instanceof Error ? error.message : "Unknown error"}`, { assetConfig });
        }
    }
    ...
    convertToDecimal(asset, rawAmount) {
        ...
        try {
            ...
            throw new liquidity_1.LiquidityException(liquidity_1.LiquidityError.INVALID_AMOUNT, `Failed to convert raw amount to decimal: ${error instanceof Error ? error.message : "Unknown error"}`, { asset, rawAmount });
        }
    }
    convertFromDecimal(asset, decimalAmount) {
        const assetConfig = this.getAssetConfig(asset);
        if (!assetConfig) {
            throw new liquidity_1.LiquidityException(liquidity_1.LiquidityError.ASSET_NOT_SUPPORTED, `Asset not supported: ${asset}`, { asset });
        }
        try {
            ...
            if (fractionalPart.length > assetConfig.decimals) {
                throw new Error(`Too many decimal places. Maximum: ${assetConfig.decimals}`);
            }
            ...
            throw new liquidity_1.LiquidityException(liquidity_1.LiquidityError.INVALID_AMOUNT, `Failed to convert decimal amount to raw: ${error instanceof Error ? error.message : "Unknown error"}`, { asset, decimalAmount });
        }
    }
    // Validation methods
    isValidAsset(asset) {
    ...
    setMinimumThreshold(asset, threshold) {
        ...
        if (!this.isValidDecimalAmount(threshold)) {
            throw new liquidity_1.LiquidityException(liquidity_1.LiquidityError.INVALID_AMOUNT, `Invalid threshold amount: ${threshold}`, { asset, threshold });
        }
    ...
    setWarningThreshold(asset, threshold) {
        const assetConfig = this.getAssetConfig(asset);
        if (!assetConfig) {
            throw new liquidity_1.LiquidityException(liquidity_1.LiquidityError.ASSET_NOT_SUPPORTED, `Asset not supported: ${asset}`, { asset });
        }
    ...
    getAssetsByNetwork(network) {
        return Array.from(this.assets.values()).filter((asset) => asset.network === network);
    }
    // Compare amounts in decimal format
    compareAmounts(asset, amount1, amount2) {
        if (!this.isValidAsset(asset)) {
            throw new liquidity_1.LiquidityException(liquidity_1.LiquidityError.ASSET_NOT_SUPPORTED, `Asset not supported: ${asset}`, { asset });
        }
        try {
            ...
            if (amount1BigInt < amount2BigInt)
                return -1;
            if (amount1BigInt > amount2BigInt)
                return 1;
            return 0;
            ...
            throw new liquidity_1.LiquidityException(liquidity_1.LiquidityError.INVALID_AMOUNT, `Failed to compare amounts: ${error instanceof Error ? error.message : "Unknown error"}`, { asset, amount1, amount2 });
        }
    }
    // Add amounts in raw format
    addAmounts(asset, amount1, amount2) {
        if (!this.isValidAsset(asset)) {
            throw new liquidity_1.LiquidityException(liquidity_1.LiquidityError.ASSET_NOT_SUPPORTED, `Asset not supported: ${asset}`, { asset });
        }
        try {
            ...
            throw new liquidity_1.LiquidityException(liquidity_1.LiquidityError.INVALID_AMOUNT, `Failed to add amounts: ${error instanceof Error ? error.message : "Unknown error"}`, { asset, amount1, amount2 });
        }
    }
    // Subtract amounts in raw format
    subtractAmounts(asset, amount1, amount2) {
        if (!this.isValidAsset(asset)) {
            throw new liquidity_1.LiquidityException(liquidity_1.LiquidityError.ASSET_NOT_SUPPORTED, `Asset not supported: ${asset}`, { asset });
        }
        try {
            ...
            throw new liquidity_1.LiquidityException(liquidity_1.LiquidityError.INVALID_AMOUNT, `Failed to subtract amounts: ${error instanceof Error ? error.message : "Unknown error"}`, { asset, amount1, amount2 });
        }
    }
}
